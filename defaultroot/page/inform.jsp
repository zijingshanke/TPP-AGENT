...
<html:base/>
<link href="../_css/stss.css" rel="stylesheet" type="text/css">
</head> 
  ...
  <tr>
    ...
    <td><table height="100" border="0" align="center" cellpadding="0" cellspacing="0" >
        <tr>
          ...
          <td valign="middle">&nbsp;&nbsp;&nbsp;
          <div align="left">
          <c:out value="${inf.message}" escapeXml="false"/></div>
          </td>
        </tr>
        <c:if test="${listRadio!=null}"> 
        <tr>
          <td colspan="2">
            <c:forEach var="tvv"  items="${listRadio}" varStatus="status"> 
              <input type="radio" name="selectRadio" value="<c:out value='${tvv.value1}'/>" <c:out value='${tvv.value2}'/>><c:out value='${tvv.title}'/><br>
            </c:forEach>
          </td>          
        </tr>
        </c:if>            
	  ...
	  <c:choose>
	   <c:when test="${inf.close==true}">  
	          ...
	          <html:link page="${inf.forwardPage}" styleId="myurl" onclick="openNewUrl()"/>
	        </c:if> 
	        <c:if test="${inf.paramId!='' and params==null}">
	          <html:link page="${inf.forwardPage}" paramId="${inf.paramId}" paramProperty="paramValue" paramName="inf" styleId="myurl" onclick="openNewUrl()"/>
	        </c:if>  
	        <c:if test="${inf.paramId=='' and params!=null}">
	           <html:link page="${inf.forwardPage}" name="params" styleId="myurl" onclick="openNewUrl()"/>
	        </c:if> 
	        <c:if test="${inf.paramId=='' and params!=null}">
	           <html:link page="${inf.forwardPage}" paramId="${inf.paramId}" paramProperty="paramValue" paramName="inf" name="params" styleId="myurl" onclick="openNewUrl()"/>
	        </c:if> 
          <a href="JavaScript:closeme()">           <c:if test="${listRadio!=null}"> 
             <html:image page="/_img/confirm.gif" border="0" onclick="addForwardPageParam()"/>
           </c:if>
           <c:if test="${listRadio==null}"> 
        ...
        <c:when test="${inf.back==false and inf.paramId=='' and params==null}">
           <html:link page="${inf.forwardPage}" styleId="myurl" onclick="openNewUrl()">  
                        <c:if test="${listRadio!=null}"> 
             <html:image page="/_img/confirm.gif" border="0" onclick="addForwardPageParam()"/>
           </c:if>
	   ...
	   <c:when test="${inf.back==false and inf.paramId!='' and params==null}">
           <html:link page="${inf.forwardPage}" styleId="myurl" paramId="${inf.paramId}" paramProperty="paramValue" paramName="inf" onclick="openNewUrl()"> 
                        <c:if test="${listRadio!=null}"> 
             <html:image page="/_img/confirm.gif" border="0" onclick="addForwardPageParam()"/>
           </c:if>
           <c:if test="${listRadio==null}"> 
	   ...
	   <c:when test="${inf.back==false and inf.paramId=='' and params!=null}">
           <html:link page="${inf.forwardPage}" styleId="myurl" name="params" onclick="openNewUrl()">  
                        <c:if test="${listRadio!=null}"> 
             <html:image page="/_img/confirm.gif" border="0" onclick="addForwardPageParam()"/>
           </c:if>
           <c:if test="${listRadio==null}"> 
       ...
       <c:when test="${inf.back==false and inf.paramId!='' and params!=null}">
           <html:link page="${inf.forwardPage}" styleId="myurl" paramId="${inf.paramId}" paramProperty="paramValue" paramName="inf" name="params" onclick="openNewUrl()"> 
           <c:if test="${listRadio!=null}"> 
             <html:image page="/_img/confirm.gif" border="0" onclick="addForwardPageParam()"/>
           </c:if>
           <c:if test="${listRadio==null}"> 
	   ...
	   <c:otherwise>
        ...
        <tr>
          <td colspan="2" align="center">
            <html:link page="${inf.blankForwardPage}" target="_blank"><c:out value="${inf.blank}"/></html:link>
          </td>
        </tr>
        </c:if>
      </table></td>
