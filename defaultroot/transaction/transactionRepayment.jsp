...
<!--JSP页面: transaction/transactionRepayment.jsp -->
	<head>
<title>钱门支付！--网上支付！安全放心！</title>
		...
		function checkForm(){
		    ...
		    if(repaymentPrice.value!=""){
				...
				if(repaymentPrice.value<=0.01||parseFloat(repaymentPrice.value)>parseFloat(price)||parseFloat(repaymentPrice.value)>parseFloat(<c:out value='${transaction.toAgent.allowBalance}'/>)){
		    ...
		    if(payPassword.value==""){
				...
				if(confirm("确定后，您将会还给<c:out value='${transaction.fromAgent.loginName}'/>( "+repaymentPrice.value+" ) 元，您确认吗？")){
	...
	<html:form method="post" action="transaction/transaction.do?thisAction=transactionRepayment">
	<div class="main_top">
      <div class="main_bottom">
        <div class="main_mid">
          <div class="main_title">
            <div class="main_title_right">
              <p><strong>还款给朋友</strong></p>
            </div>
          </div>
          <table cellpadding="0" cellspacing="0" width="100%" class="information_table">
            <tr>
              <td class="right_td">您的钱门账户：</td>
              <td style="color:#005C9C"><c:out value="${transaction.toAgent.loginName}" /></td>
            </tr>
            <tr>
              <td class="right_td">借款方钱门账户：</td>
              <td><c:out value="${transaction.fromAgent.loginName}"></c:out> </td>
            </tr>
           <tr>
              <td class="right_td">已还金额：</td>
              <td style="color:#005C9C">
              <c:if test="${sumRepaymentPrice>0.00}">
             	<fmt:formatNumber value="${sumRepaymentPrice}" pattern="0.00" /> 元
              </c:if>
              <c:if test="${sumRepaymentPrice==0.00}">
              0.00 元
              </c:if>
            ...
            <tr>
            	...
            	<td style="color:red">
            	<fmt:formatNumber value="${repaymentAmaut}" pattern="0.00" /> 元</td>
            </tr>
             <tr>
              <td class="right_td"><span class="font_style6">* </span>还款原因：</td>
              <td><input type="text" class="text_style" name="orderDetails.paymentReason" id="paymentReason" value="还“<c:out value="${transaction.orderDetails.shopName}"/>”(<c:out value="${transaction.orderDetails.orderDetailsNo}"/>)"/></td>
            </tr> 
            <tr>
              <td class="right_td"><span class="font_style6">* </span>还款金额：</td>
              <td><input type="text" class="text_style" style="width:70px;" name="repaymentPrice" id="repaymentPrice" maxlength="12"  onKeyUp="value=value.replace(/[^.0-9]/g,'')" onchange="value=value.replace(/[^.0-9]/g,'')" value="<fmt:formatNumber value="${repaymentAmaut}" pattern="0.00" />"/> 元  <div id="shopPriceDiv" style="color: red"></div></td>
            </tr> 
            <tr>
