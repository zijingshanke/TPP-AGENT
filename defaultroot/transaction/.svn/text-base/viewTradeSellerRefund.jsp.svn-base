...
<!--JSP页面: transaction/viewTradeSellerRefund.jsp-->
    ...
    <div class="refundAgreement-right">
		<p>对方账户：<b class="txt-TIME-s">
		<c:if test="${fg==1}">
			<c:out value="${transaction.fromAgent.name}"/> (<c:out value="${transaction.fromAgent.loginName}"/>)
		</c:if>
		<c:if test="${fg==0}">
			<c:out value="${transaction.toAgent.name}"/> (<c:out value="${transaction.toAgent.loginName}"/>)
		</c:if>
		</b>
		</p>
		<p>操作提示</p>
