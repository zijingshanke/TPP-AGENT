...
<!--JSP页面: transaction/transactionLetterAndRepayment.jsp -->
...
<script language="javascript">
		...
		function getTransactionSuccessAndFailDetail(tid,statusid,orderid,flag,agentid,type){
			var allLetter = document.all.allLetter.value;
			window.location.href="../transaction/transaction.do?thisAction=getTransactionSuccessAndFailDetail&flag="+flag+"&type="+type+"&agentid="+agentid+
			"&statusid="+statusid+"&tid="+tid+"&orderid="+orderid+"&allLetter="+allLetter;
		}
		...
		function creditRepayment(tid){
			window.location.href="../transaction/transaction.do?thisAction=creditRepayment&tid="+tid;
		}
		...
		function chooseType(){
			...
			window.location.href="../transaction/transactionlist.do?thisAction=getLetterAndRepaymentList&agentType="+agentType+"&menuList="+menuList+"&status="+status;
		}
...
<html:form action="transaction/transactionlist.do" method="post">
	...
	<a id="Show_10"	onclick="AllTransaction();" style="cursor:hand">所有的交易</a>
	...
	<a id="Show_17" onclick="LoaningOrRepayment();" class="at_deal_list_head" style="cursor:hand">授信/还款</a>
		 <a id="Show_18" onclick="RedPacket();" style="cursor:hand">红包</a>
	</div>
	<div class="deal_list_count">当前显示：<select name="agtype"
	...
	<table cellpadding="0" cellspacing="0" width="100%" class="deal_table" style="word-break :break-all">
		...
		<c:forEach var="info" items="${tlf.list}" varStatus="statu">
			<tr>
				...
				<td align="center"><c:out value="${info.typeCaption}"/></td>
				...
				<c:if test="${info.type==102||info.type==103}">
					<td></td>
				</c:if>
				<c:if test="${info.type==101||info.type==180}">
				<c:if test="${(info.type==101&&info.actionCaption=='卖出')||info.type==180}">
					<td align="center">
						<c:out value="${info.orderDetails.isRepay }" />
					</td>
				</c:if>
				<c:if test="${!(info.type==101&&info.actionCaption=='卖出'||info.type==180)}">
					<td align="center">
				...
				<td><!-- 可执行操作开始 --> <c:forEach var="lf"
					...
					<c:if test="${info.type==102||info.type==103||info.type==180}">
						<a href="<c:out value="${lf.url}"/>"
							title="<c:out value="${lf.title}"/>"
							style='c: out value = "${lf.style}"/ &gt;'><c:out
							value="${lf.urlName}" /></a>
					</c:if>
					<c:if test="${info.type==101&&(info.actionCaption=='买入'||info.orderDetails.isRepay=='未还款') }">
						<a href="<c:out value="${lf.url}"/>"
							title="<c:out value="${lf.title}"/>"
							style='c: out value = "${lf.style}"/ &gt;'><c:out
							value="${lf.urlName}" /></a>
		...
		<input type="hidden" id="allLetter" value="allLetter" />
		<tr>
			
			<td colspan="12" class="page_td">
...
<script language="javascript">
	...
	//alert(tabid_num+tabnum);
		for(var i=0;i<7;i++){document.getElementById("Show_"+tabid_num+i).className="";}
