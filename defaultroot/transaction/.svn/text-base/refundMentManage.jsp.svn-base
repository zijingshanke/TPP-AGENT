...
<!--JSP页面: transaction/refundMentManage.jsp -->
...
<div class="main_title2"><span class="right_span">我的账户：<c:out
	...
	class="font_style7">交易查询：</span>(按交易创建日期查询)</div>
		  <html:form action="transaction/transactionlist.do?showTabMenuIdList=1,3&showSubMenuIdList=0,4,0,3" method="post">
		...
		<tr>
			...
			<!-- <td><input type="text" class="text_style" name="searchAgentName" value="<c:out value="${tlf.searchAgentName}"/>" /></td> -->
			<td><html:text property="searchAgentName" styleId="searchAgentName" styleClass="text_style"></html:text> </td>
		</tr>
		<tr id="orderNoTR">
			<td class="right_td">交易号：</td>
			<!--  <td><input type="text" class="text_style" id="searchOrderNo" name="searchOrderNo" value="" /></td>-->
			<td><html:text property="searchOrderNo" styleId="searchOrderNo" styleClass="text_style"></html:text> </td>
		</tr>
		<tr id="outOrderNoTR">
			<td class="right_td">商品订单号：</td>
			<!--<td><input type="text" class="text_style" id="searchOutOrderNo" name="searchOutOrderNo" value="<c:out value="${tlf.searchOutOrderNo}"/>" /></td>-->
		...
		<tr id="searchShopNameTR">
			...
			<!-- <td><input type="text" class="text_style" id="searchShopName" name="searchShopName" value="<c:out value="${tlf.searchShopName}"/>" /></td>-->
			<td><html:text property="searchShopName" styleId="searchShopName" styleClass="text_style"></html:text> </td>
		</tr>
		<tr id="searchMoneyTR">
			<td class="right_td">交易金额：</td>
			<td><input type="text" class="text_style" id="searchBeginMoney"
				name="searchBeginMoney" onfocus="this.value=''"
				value="<c:out value="${tlf.searchBeginMoney}"/>" /> -- <input
				type="text" class="text_style" id="searchEndMoney" onfocus="this.value=''"
				name="searchEndMoney" value="<c:out value="${tlf.searchEndMoney}"/>" /></td>
		</tr>
		<tr>
          ...
          <div class="deal_list">
            ...
            <table cellpadding="0" cellspacing="0" width="100%" class="deal_table">
              ...
              <tr>
                ...
                <c:if test="${info.flag==1}">
                <td align="center"><fmt:formatNumber pattern="0.00" value="${info.amount}"/></td>
                <td align="center"></td>
                </c:if>
                <c:if test="${info.flag==0}">
                <td align="center"></td>
                ...
                <td>
					...
					<c:if test="${info.flag==1 && info.status==7}">
		 			...
		 			<c:if test="${info.status==11}">
		 				<a href="javascript:getRefundMentBeforeShippingDetail('<c:out value="${info.id}" />','<c:out value="${info.orderDetails.id}" />',3,'<c:out value="${info.flag}" />')">查看</a>
		 			</c:if>
		 		</td>
...
<script language="javascript">
	...
	function Show_Menu(tabid_num,tabnum){
     ...
     function AllTransaction(){
		    Show_Menu(1,0);
			document.forms[0].action="../transaction/transactionlist.do?thisAction=listTransactions&showTabMenuIdList=1,3&showSubMenuIdList=0,4,0,3&menuList=1,0&flag=1&status=''";
			smb();
		}
	function Transactions(){
		    Show_Menu(1,1);
			document.forms[0].action="../transaction/transactionlist.do?thisAction=listTransactions&showTabMenuIdList=1,3&showSubMenuIdList=0,4,0,3&menuList=1,1&status=1&flag=1";
			smb();
		}
	function SuccessTransaction(){
	    Show_Menu(1,2);
		document.forms[0].action="../transaction/transactionlist.do?thisAction=listTransactions&showTabMenuIdList=1,3&showSubMenuIdList=0,4,0,3&menuList=1,2&status=3&flag=1";
		smb();
	}
  function FailureTransaction(){
	    Show_Menu(1,3);
		document.forms[0].action="../transaction/transactionlist.do?thisAction=listTransactions&showTabMenuIdList=1,3&showSubMenuIdList=0,4,0,3&menuList=1,3&status=4&flag=1";
		smb();
	}
