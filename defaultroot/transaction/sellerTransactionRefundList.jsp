...
<!--JSP页面: transaction/sellerTransactionRefundList.jsp -->
...
<div class="main_title2"><span class="right_span">我的账户：<c:out
	...
	class="font_style7">交易查询：</span>(按交易创建日期查询)</div>
		  <html:form action="transaction/transactionlist.do?thisAction=getSellerTransactionRefundList&fg=seller" method="post">
		...
		<tr>
          ...
          <div class="deal_list">
            <div class="deal_list_head">
              <a id="Show_10" onclick="AllTransaction();" style="cursor:hand">所有卖出交易</a>
              ...
              <tr>
                ...
                <td>
					...
					<c:if test="${info.flag==1 && info.status==7}">
		 			...
		 			<c:if test="${info.status==11}">
		 				<a href="javascript:viewTradeSellerRefund('<c:out value="${info.id}" />','<c:out value="${info.orderDetails.id}" />',3,'<c:out value="${info.flag}" />')">查看</a>
		 			</c:if>
		 		</td>
...
<script language="javascript">
	...
	function AllTransaction(){
		    Show_Menu(1,0);
			document.forms[0].action="../transaction/transactionlist.do?thisAction=getSellerAndBuyerTransactionList&fg=seller&showTabMenuIdList=1,3&showSubMenuIdList=0,4,0,3&menuList=1,0&flag=1&status=''";
			smb();
		}
	function Transactions(){
		    Show_Menu(1,1);
			document.forms[0].action="../transaction/transactionlist.do?thisAction=getSellerAndBuyerTransactionList&fg=seller&showTabMenuIdList=1,3&showSubMenuIdList=0,4,0,3&menuList=1,1&status=1&flag=1";
			smb();
		}
	function SuccessTransaction(){
	    Show_Menu(1,2);
		document.forms[0].action="../transaction/transactionlist.do?thisAction=getSellerAndBuyerTransactionList&fg=seller&showTabMenuIdList=1,3&showSubMenuIdList=0,4,0,3&menuList=1,2&status=3&flag=1";
		smb();
	}
  function FailureTransaction(){
	    Show_Menu(1,3);
		document.forms[0].action="../transaction/transactionlist.do?thisAction=getSellerAndBuyerTransactionList&fg=seller&showTabMenuIdList=1,3&showSubMenuIdList=0,4,0,3&menuList=1,3&status=4&flag=1";
		smb();
	}
