...
<!--JSP页面: transaction/buyerTransaction.jsp -->
...
<script language="javascript">
		...
		function getTransactionSuccessAndFailDetail(tid,statusid,orderid,flag,agentid,type){
			window.location.href="../transaction/transaction.do?thisAction=getTransactionSuccessAndFailDetail&flag="+flag+"&type="+type+"&agentid="+agentid+"&statusid="+statusid+"&tid="+tid+"&orderid="+orderid;
		}
		
		function viewTradeBuyTransactionSuccess(tid,statusid,orderid,flag,agentid,type){
			window.open("../transaction/transaction.do?thisAction=viewTradeBuyTransactionSuccess&flag="+flag+"&type="+type+"&agentid="+agentid+"&statusid="+statusid+"&tid="+tid+"&orderid="+orderid);
		}
		
		function transactionDetailByDunAndBatch(tid,statusid,orderid,flag,agentid,type){
			window.location.href="../transaction/transaction.do?thisAction=transactionDetailByDunAndBatch&flag="+flag+"&type="+type+"&agentid="+agentid+"&statusid="+statusid+"&tid="+tid+"&orderid="+orderid+"&seller=1";
		}
		...
		function viewTradeSellerRefund(tid,orderid,flag){ 
			window.open("../transaction/transaction.do?thisAction=viewTradeSellerRefund&tid="+tid+"&orderid="+orderid+"&flag="+flag);
		}
		...
		function offLineRefund(tid,orderid,type){
			if (!confirm("确实要退回这笔款吗？")) return;
			window.location.href="../transaction/transaction.do?thisAction=offLineRefund&tid="+tid+"&orderid="+orderid+"&type="+type;
		}
...
<html:form action="transaction/transactionlist.do?thisAction=getSellerAndBuyerTransactionList&fg=buyer" method="post">
	...
	<table cellpadding="0" cellspacing="0" width="100%"
		...
		<tr id="searchMoneyTR">
			...
			<td>
			<html:text property="searchBeginMoney" onfocus="this.value=''" styleId="searchBeginMoney" styleClass="text_style" onkeyup="value=value.replace(/[^.0-9]/g,'')" onchange="value=value.replace(/[^.0-9]/g,'')"></html:text>
			--
			<html:text property="searchEndMoney" onfocus="this.value=''" styleId="searchEndMoney" styleClass="text_style" onkeyup="value=value.replace(/[^.0-9]/g,'')" onchange="value=value.replace(/[^.0-9]/g,'')"></html:text>
			</td>
		</tr>
		<tr>
	...
	<a id="Show_10"	onclick="AllTransaction();"	class="at_deal_list_head" style="cursor:hand">所有买入交易</a> 
...
<script language="javascript">
   ...
   function AllTransaction(){
		    Show_Menu(1,0);
			document.forms[0].action="../transaction/transactionlist.do?thisAction=getSellerAndBuyerTransactionList&fg=buyer&menuList=1,0&flag=1&status=''";
			document.forms[0].submit();
		}
	function Transactions(){
		    Show_Menu(1,1);
			document.forms[0].action="../transaction/transactionlist.do?thisAction=getSellerAndBuyerTransactionList&fg=buyer&menuList=1,1&status=1&flag=1";
			document.forms[0].submit();
		}
	function SuccessTransaction(){
	    Show_Menu(1,2);
		document.forms[0].action="../transaction/transactionlist.do?thisAction=getSellerAndBuyerTransactionList&fg=buyer&menuList=1,2&status=3&flag=1";
		document.forms[0].submit();
	}
  function FailureTransaction(){
	    Show_Menu(1,3);
		document.forms[0].action="../transaction/transactionlist.do?thisAction=getSellerAndBuyerTransactionList&fg=buyer&menuList=1,3&status=4&flag=1";
		document.forms[0].submit();
	}
