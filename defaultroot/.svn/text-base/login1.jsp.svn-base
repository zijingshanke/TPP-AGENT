...
{
	detectPlugin("C844E2C9-1921-46FB-A564-0CCDB3116844","CheckLocalCert2QM");
}
function winopen(a)
{
	window.open(a,"alank","width=600,height=400,scrollbars=yes,resizable=no,toolbar=no,directories=no,menubar=no,top=100,left=200");
   	...
   	function downloadActiveX(){
   		...
   		winopen("<%=request.getContextPath()%>/CertOcx_1.3.exe");
   		}else{
   		alert("ss");
			}
}
...
var formid = document.getElementById("f1");
var myControl2=document.getElementById("myControl2");
if(myControl2.CheckLocalCert2QM!=undefined){
...
<body onload="checkActivex()">
  <div id="warp"><br/>
    <div id="head">
      ...
      <img src="<%=request.getContextPath()%>/_img/head_img.gif" class="menu_img" />
    </div>
    <div id="main">
        ...
        <form id="f1" action="<%=request.getContextPath()%>/agent/agent.do?thisAction=login" method="post">
         <html:hidden property="forwardpage" value="${param.forwardpage}"/>
          <table cellpadding="0" cellspacing="0" width="225">
            ...
            <c:if test="${URI==null}">
            <tr>
              <td><input name="loginName" id="loginName" value="" type="text"  rule="notnull" style="width:180px;border:1px solid #999;" onkeydown="if(event.keyCode==13){toSubmit();}" /></td>
            </tr>
            <tr>
              <td>登录密码：</td>
            </tr>
            <tr>
              ...
              <div id="objectActivex" style="display: block;">
                                 <object id="myControl2" name="myControl2"  classid="clsid:C844E2C9-1921-46FB-A564-0CCDB3116844"
     codebase="certOcx.cab" width="0px;" height="0px;" style="display: none"
>
       </object><input name="password" type="password" id="password" value="" rule="length:6:20" style="width:180px;border:1px solid #999;" onkeydown="if(event.keyCode==13){toSubmit();}" />
              </div>
