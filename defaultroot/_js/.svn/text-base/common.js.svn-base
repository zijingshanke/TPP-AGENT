...
function removeLetter(str)
{
 ...
 function isAssetsNo(str)
 {  
   var pattern = /^[1-9][0-9]{2}[0-9]{2}([A-Z]{2})?([0-9]{2})?([0-9]{4})?$/;
   return pattern.test(str)
 }
...
function isIE()
{
  ...
  { 
    var height=_height;
    var width=_width;
    return window.showModalDialog(_url,par,"dialogwidth:"+_width+"px;dialogheight:"+_height+"px;center:yes;status:no;scroll:no;help:no");
  } 
...
function showElement(str){
 ...
 function required(_obj){
	if(_obj==null||typeof(_obj)=="undefined"||_obj.value.trim()==""){
		return false;
	}
	return true;
}
  //20090507-ref001-jason-e
  //20090508-ref001-guanhao-s
 
//20090508-ref001-guanhao-e