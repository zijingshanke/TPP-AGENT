...
var DIGIT = 2;
var PUNCTUATION = 3; 
var ASIA_CHAR = 4;  //双字节字符，ascii<0 或 >255
...
{    
    return (IsLong(IsSideStrongPassword.arguments[0], "7") &&
...
{
    return (IsLong(IsMediumPassword.arguments[0], "7") &&
...
function printIntensity(pwd) 
{
    if(!(document.getElementById(td_weak)&&document.getElementById(td_medium)&&document.getElementById(td_strong)&&document.getElementById(pwd))){
	return;
	}
	var td_weak = "td_weak_" + pwd;
    var td_medium = "td_medium_" + pwd;    
    var td_strong = "td_strong_" + pwd;
    
    var pwdVal = 	document.getElementById(pwd).value;
	
    if(ByteLength(pwdVal) < 6) {
	        document.getElementById(td_weak).className = "td_init_intensity";
	        document.getElementById(td_medium).className = "td_init_intensity";
	        document.getElementById(td_strong).className = "td_init_intensity";       	
    } else {
    var v = PwdIntensity(pwdVal);
	    if(v==3){
	        document.getElementById(td_weak).className = "td_init_intensity";
	        document.getElementById(td_medium).className = "td_init_intensity";
	        document.getElementById(td_strong).className = "td_strong_intensity";
	    }    
	    else if(v==2){
	        document.getElementById(td_weak).className = "td_init_intensity";
	        document.getElementById(td_medium).className = "td_medium_intensity";
	        document.getElementById(td_strong).className = "td_init_intensity";        
	    }   
	    else if(v==1){
	        document.getElementById(td_weak).className = "td_weak_intensity";
	        document.getElementById(td_medium).className = "td_init_intensity";
	        document.getElementById(td_strong).className = "td_init_intensity";        
	    }   
	    else if(v==0){
