					...
					<div class="main_mid">
						<div class="main_title">
							<div class="main_title_right">
								<p>
									<strong>来自&nbsp;<c:out
											value="${GatewayForm.partnerInfo.name}" />&nbsp;的信用支付（外买）交易。</strong>
								</p>
							</div>
						</div>
						...
						<div class="deal_list" style="position: relative;">
							...
							<div id="head2" style="display:none;" class="deal_list_head">
								<a id="tab_3"  href="#" class="at_deal_list_head">数字证书</a>
							</div>
								<!-- 钱门信用金额付款 START -->
								<div class="deal_list_count deal_list_count_1"
									id="dummy_payment">
									<html:form action="/cooperate/credit.do" method="post"
										onsubmit="return goPayment()">
										<html:hidden property="url"  />
										<html:hidden property="service" />
										<!--<html:hidden  property="buyerEmail" value="${GatewayForm.buyerEmail}" />-->
										<html:text property="buyerEmail" styleId="payemail1" style="display:none"  indexed="1" value="${GatewayForm.buyerEmail}">
													</html:text>
										<table cellpadding="0" cellspacing="0" width="100%"
											class="information_table">
											<tr>
...
function goPayment(){
	with(document.forms[0]){
		...
		service.value="credit_paymentout";
	}
	return true;
}
