 ...
 * @author YanRui
 */
public class TestLogUtil extends FileUtil{
	 ...
	 */
	@SuppressWarnings("unchecked")
	public TestLogUtil(boolean isSystemOut, boolean isPrintClass, Class classIn) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public TestLogUtil(boolean isSystemOut, boolean isPrintClass,
	...
	public String getLogInfo(int logLevel, String logInfo, Class classIn) {
		...
		if (checkLogLevel(logLevel)) {// 检查日志等级设置
			...
			if (logLevel == logLevel_Error) {
				...
				logInfo = TestLogUtil.class.getName() + "打印日志未正确设置日志信息等级 "
						+ dateStr + "] " + logInfo;
			}
		} else {
			logInfo = TestLogUtil.class.getName() + "打印日志未正确设置日志信息等级 "
