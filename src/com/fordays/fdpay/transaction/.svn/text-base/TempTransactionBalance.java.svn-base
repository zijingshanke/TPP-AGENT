...
public class TempTransactionBalance
{
	...
	public TempTransactionBalance(long id, Long transactionId,
      Long transactionType, Long transactionStatus, BigDecimal balance,
      BigDecimal notallowBalance,BigDecimal creditBalance, Long agentId, Long status,
      ...
      String toAgentEmail, String shopName, String remark,Long buyType)
  {
	  super();
	  this.id = id;
	  this.transactionId = transactionId;
	  ...
	  if(orderNo!=null&&orderNo.indexOf("~")!=-1&&orderNo.indexOf("~")==orderNo.lastIndexOf("~")){
	  		this.orderNoSplit=(orderNo.substring(0,orderNo.indexOf("~")));
	  	}else{
	  		this.orderNoSplit=orderNo;
	  	}
	  this.amount = amount;
...
//	}
	...
	public List getOperate()
	{
		...
		if(transactionStatus==3){
			url = "javascript:getTransactionSuccessAndFailDetail('" + transactionId
		    + "','" + transactionStatus + "','" + orderDetailsId + "','0','"
		    + fromAgentId + "','" + transactionType + "')";
		}
	...
	public void setOrderNo(String orderNo)
  {
  	this.orderNo = orderNo;
  	if(orderNo!=null&&orderNo.indexOf("~")!=-1&&orderNo.indexOf("~")==orderNo.lastIndexOf("~")){
  		setOrderNoSplit(orderNo.substring(0,orderNo.indexOf("~")));
  	}else{
  		setOrderNoSplit(orderNo);
  	}
	...
	public String getTypeCaption()
  {
	...
	public String getBuyTypeCaption(){
        return OrderDetails.getBuyTypeCaption(this.getBuyType());
	}
