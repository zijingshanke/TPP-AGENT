...
public class TaskTimestampDAOImpl extends BaseDAOSupport implements
	...
	public TaskTimestamp getTaskTimestamp(Agent agent, Long taskType) throws AppException {
		Hql hql = new Hql();
		//实名认证任务没有时间限制
		if(TaskTimestamp.type_6==taskType||TaskTimestamp.type_7==taskType){
	...
	public void deleteById(long id) throws AppException
	{
		if (id > 0)
		{
			TaskTimestamp taskTimestamp = (TaskTimestamp) this.getHibernateTemplate().get(TaskTimestamp.class,
			    new Long(id));
			this.getHibernateTemplate().delete(taskTimestamp);
