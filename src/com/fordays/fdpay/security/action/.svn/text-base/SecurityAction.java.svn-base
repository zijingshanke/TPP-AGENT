...
public class SecurityAction extends DispatchAction
{
	    ...
	    throws Exception
	{
		...
		SecurityActionForm actionForm = (SecurityActionForm) form;
		// 如果未绑定手机,则跳转到绑定手机页面
		...
		if (agent != null && agent.getId() > 0)
		{
			...
			actionForm.setMobile(agent.getMobile());
			actionForm.setEmail(agent.getEmail());
		}
		
		System.out.println("*****************");
		request.setAttribute("SecurityActionForm", actionForm);
	    ...
	    throws Exception
	{
		...
		if (mobile != "" && mobile != null)
		{
			try
			{
				...
				SMUtil.send(mobile, message + randomMsg + "，请正确填写验证码，完成数字证书的申请 【钱门】 ");
	    ...
	    throws Exception
	{
		...
		SecurityActionForm securityactionForm = (SecurityActionForm) form;

		HttpSession session = request.getSession();
		System.out.println("是申请还是备份？" + securityactionForm.getType());
		session.setAttribute("SecurityActionForm", securityactionForm);
		try
		{
...
//		Agent agent = uri.getAgent();
//		SecurityActionForm securityForm = (SecurityActionForm) form;
//		SecurityActionForm securityactionForm = (SecurityActionForm) session
//		    .getAttribute("SecurityActionForm");
//		String password = securityForm.getPassword();
//		String ip = request.getRemoteHost();
//		// response.setContentType("application/p12-type");
//		String confirmPwd = request.getParameter("confirmPwd");
//		System.out.println("  password--" + password + "  confirmPwd--"
//		    + confirmPwd);
//		try
//		{
//			if ("create".equals(securityForm.getType()))
//			{
//
//				CertInfo certInfo = certInfoBiz.createUpCert(securityactionForm, agent,
//				    ip); // 备份数字证书前的工作
//				if (certInfo != null)
//				{
//					session.setAttribute("certmsg", "证书生成!");
//					System.out.println("是否保存证书会话信息:--"
//					    + session.getAttribute("certmsg").toString());
//					return mapping.findForward("backUpCert");
//				}
//			}
//			if ("download".equals(securityForm.getType()))
//			{
//				if (confirmPwd.equals(password))
//				{
//					securityactionForm.setPassword(password);
//					CertInfo certInfo = certInfoBiz.backUpCert(securityactionForm, agent,
//					    ip, request);
//					String email = certInfo.getEmail();
//					email = java.net.URLEncoder.encode(email, "UTF-8");
//					String url = "http://qm.qmpay.com/security/certificate.do?thisAction=downloadP12";
//					// Certification.getP12Cert(certInfo);
//					response.sendRedirect(url + "&email=" + email);
//					// URLUtil.getResponseBodyAsGet(url+"&email="+email).toString();
//				}
//
//			}
//
//			// if (confirmPwd.equals(password)) {
//			// securityactionForm.setPassword(password);
//			// CertInfo certInfo=certInfoBiz.backUpCert(securityactionForm,
//			// agent, ip); // 备份数字证书前的工作
//			// String email=certInfo.getEmail();
//			// email=java.net.URLEncoder.encode(email, "UTF-8");
//			// String url =
//			// "http://qm.qmpay.com/security/certificate.do?thisAction=downloadP12";
//			// //String cmd= Certification.getP12Cert(certInfo);
//			// response.sendRedirect(url+"&email="+email);
//			//							
//			// }
//		}
...
//
	 ...
	 * 确认注销证书申请
	 */
	public ActionForward verifyRevokeCert(ActionMapping mapping, ActionForm form,
	    HttpServletRequest request, HttpServletResponse response)
	    throws Exception
	{
		UserRightInfo uri = (UserRightInfo) request.getSession()
		    .getAttribute("URI");
		String mobile = uri.getAgent().getMobile();
		...
		try
		{
			...
			SMUtil.send(mobile, message + randomMsg + "，请输入正确的验证码完成注销【钱门】");
			request.getSession().setAttribute("RANDOM_MSG", randomMsg);
			System.out.println("手机验证码信息:--" + message + randomMsg);
	    ...
	    throws Exception
	{
		...
		try
		{
			CertInfo userCertInfo = certInfoBiz.reviewCertificate(uri.getAgent());
	...
	private String dealwithString(String fileRoute)
	{
		...
		String fileType = fileName.substring(f1, fileName.length()).toLowerCase();
		System.out.println("证书类型：" + fileType);
		String newString = "";
		if (fileType.equals("p12"))
		{
			if (fileRoute.indexOf(" ") != -1)
			{// 如果路径字符串中有空格，则做处理
				char a = '"';
	...
	{
		HttpSession session = request.getSession();
		String msg1 = new String(request.getParameter("msg1")
		    .getBytes("ISO-8859-1"), "UTF-8"); // js 单次编码 取值的方式
		...
		// String message=new
		// String(request.getParameter("msg1").getBytes("ISO-8859-1"),"UTF-8");
		// String message2=new
		// String(request.getParameter("msg2").getBytes("ISO-8859-1"),"UTF-8");
		// System.out.println("new String 方式:"+message+" message2="+message2);
		response.setContentType("text/xml;charset=GBK");
		response.setHeader("Cache-Control", "no-cache");
		StringBuffer sb = new StringBuffer();
		sb.append("<?xml version=\"1.0\" encoding=\"GBK\" ?> ");
	    ...
	    throws AppException
	{
		...
		else
		{
			...
			else
			{
				...
				else if (tempagent.getPassword().equals(MD5.encrypt(password)))
				{
	    ...
	    throws AppException
	{
		...
		else
		{
			...
			else
			{
				...
				else if (tempagent.getPayPassword().equals(MD5.encrypt(password)))
				{
	...
	public ActionForward editQuestionAsk(ActionMapping mapping, ActionForm form,
	    HttpServletRequest request, HttpServletResponse response)
	{
		UserRightInfo URI = (UserRightInfo) request.getSession()
		    .getAttribute("URI");
	    ...
	    throws AppException
	{
		...
		String question = "我" + firstquestion[0] + "的" + secondtquestion[0] + "是?";
		String question1 = "我" + firstquestion[1] + "的" + secondtquestion[1] + "是?";
		String question2 = "我" + firstquestion[2] + "的" + secondtquestion[2] + "是?";
		...
		for (int i = 0; i < 3; i++)
		{
			if (firstquestion[i].equals("请选择") || secondtquestion[i].equals("请选择"))
			{
	...
	public ActionForward securityChecked(ActionMapping mapping, ActionForm form,
	    HttpServletRequest request, HttpServletResponse response)
	    throws AppException
	{
		UserRightInfo uri = (UserRightInfo) request.getSession()
