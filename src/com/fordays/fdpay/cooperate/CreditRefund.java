...
public class CreditRefund implements Runnable {
	GatewayBiz biz;
	...
	public CreditRefund(GatewayBiz biz) {
		this.biz = biz;
	}
	...
	public void run() {
		try {
			System.out.println("信用支付还款开启线程....");
			String result=biz.credit_refund(map, url, gatewayMessage);
			for(int i=0;i<3;i++){
				try {
