...
public class BOCOMB2CClient {
	...
	private com.bocom.netpay.b2cAPI.B2CConnection connection;
	...
	public String executeOperation(String opName, String reqData) {
		myLog = new LogUtil(false, false, BOCOMB2CClient.class);
		// System.out.println("===BOCOMB2CClient execcuteOperation=====");
		...
		try {
			...
			String fullURL = BOCOMSetting.ApiURL + "?dse_operationName="
					+ opName;
			// System.out.println("1------fullURL-----" + fullURL);

			fullURL = fullURL + "&reqData=" + URLEncoder.encode(reqData);
			// System.out.println("2------fullURL-----" + fullURL);
			if (connection == null)
				connection = new B2CConnection(BOCOMSetting.UseSSL);
